Feature: Test Debenhams minibag feature for International site(Australia)

@automated @DDAI-304
Scenario: Navigate to Debenhams home page>Verify Empty minibag feature
...
@automated @DDAI-304
Scenario: Navigate to Debenhams home page>MiniBag>Validate product details on minibag for Australia
  #Given I navigate to "homeURL"
  #And I wait for "1000" milliseconds
...
@manual @DDAI-304
Scenario: Navigate to Debenhams home page>MiniBag>More than 6 items to bag functioanlity for Australia
...
@manual @DDAI-304
Scenario: Navigate to Debenhams home page>MiniBag>Verify minibagicon number once the product is added or removed for Australia
  Given I have added or removed a new product to the bag
  Then the mini bag icon number will update

@manual @DDAI-304
Scenario: Navigate to Debenhams home page>MiniBag>Verify closeicon on minibag for Australia
  Given I have the mini bag open
  When I press the x
  Then the mini bag will close

@manual @DDAI-304
Scenario: Navigate to Debenhams home page>MiniBag>clicking on Secure Checkout navigate to my bag page for Australia
...
@manual @DDAI-304
Scenario: Navigate to Debenhams home page>MiniBag>Out of Stock for Australia
...
@manual @DDAI-304
Scenario: Navigate to Debenhams home page>MiniBag>Back to top for Australia
