...
Scenario: Verify Collection Location for Debs store
...
@automatedPudoSanity @PUDO @DEBD-3230
Scenario: Verify that user is able to see a Map showing the pointer and Debehams logo, Debenhams logo, Store name, distance in miles, store address,Collection location, Change store CTA & Delivery lead time when a Debenhams store is choosen as collection store
    And I click on "listView" on "locateStorePage"
...
Scenario Outline: Verify that user is able to plcae order with debs store delivery selected
