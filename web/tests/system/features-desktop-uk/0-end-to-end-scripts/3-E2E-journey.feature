...
Scenario: Navigate to chanel perfume PDP page and validate the quantity selector is disabled when quantity set to 3 (max quantity)
  ...
  Then I should verify "font-weight" of "chanelPdpTitle" as "700" on "pdpPage"
  Then I should verify "color" of "chanelPdpTitle" as "rgba(0, 0, 0, 0)" on "pdpPage"
...
Scenario: Add quantity 2 and add to bag and store details from my bag
  Then I should verify "background-color" of "chanelAddToBagCta" as "rgba(0, 0, 0, 0)" on "pdpPage"
  And I select save item for the chanel product on pdpPage
  Then I should verify "color" of "chanelSflButton" as "rgba(0, 0, 0, 0)" on "pdpPage"
...
Scenario: Navigate to home delivery options by filling the UK address manually
  ...
  Then I should see "nextNominatedOptionUk" exist on "deliveryOptionPage"
...
Scenario: Validate the options under Next nominated delivery option and select the first available daytime option
  When I click on "nextNominatedOptionUk" on "deliveryOptionPage"
...
@nautomatedSanity  @preprodSanity @e2e
