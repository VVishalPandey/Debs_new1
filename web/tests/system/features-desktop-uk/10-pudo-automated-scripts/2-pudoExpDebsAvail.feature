...
Scenario: Verify the Collection Location section for Hermes Store
...
@automatedPudoSanity @PUDO @DEBD-3230
Scenario: Verify that user is getting only one (Express PUDO) delivery option for selected Hermes store
...
Scenario Outline: Verify that user is able to place order with Express PUDO store delivery
    Then I "select" "Nisa Local" store "1.04 Miles" from "locateStorePage"
    And I click on "selectStoreButton" on "locateStorePage"
    Then I click on "pudoExpressRadioButton" on "deliveryOptionPage"
    Then I click on "proceedPaymentButton" on "deliveryOptionPage"
...
Scenario Outline: Add DebParcelStoreExpressCC product to bag and naviagte to Delivery Page
    Given I navigate to "pDpDsEPsAvailableUrl"
    Then I select available size and add to bag on pdpPage
    When I click on "viewBasketCheckoutButton" on "pdpPage"
    Then I should see "myBagPageHeader" exist on "myBagPage"
