...
Scenario: Navigate to Debenhams Debenhams payment page and validate the page after session timeout
  ...
  And I navigate to payment page
  And I leave system ideal for 30 minutes for session time out
  Then I should try to pay with beauty club card
  Then I should see "Welcome back" page
  Then i entered username and password
  And I click on sign in link
  Then I navigate to payment page
...
@manual @DDAI-1024 @awsManual
Scenario: Validate on removing the Beauty points after session time out , user should be taken to the partial login page.
	Given the user is not logged in
  And the user adds the beauty products to the basket page
	And the user navigates to Payment page
	And the user add a "BeautyPointID" in "BeautyPointIDField" on the payment page
  Then the user is inactive for 30min
	When the user removes the applied "BeautyPointID" on payment page
  Then the user should be taken back to the "Welcome Back" page

@manual @DDAI-1024 @awsManual
Scenario: Validate on removing the Beauty reward card after session time out , user should be taken to the partial login page.
	Given the user is not logged in
  And the user adds the beauty products to the basket page
	And the user navigates to Payment page
	And the user add a "BeautyCardDetails" on the payment page
  Then the user is inactive for 30min
	When the user removes the applied "BeautyCardDetails" on payment page
  Then the user should be taken back to the "Welcome Back" page
