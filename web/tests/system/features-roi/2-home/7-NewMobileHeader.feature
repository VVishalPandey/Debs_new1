Feature: Validate new mobile header - SFL heart

@manual @DDAI-917
Scenario: Validate header on home page as registered user
...
@manual @DDAI-917
Scenario: Validate header on dress psp page as guest user 
  Given I am on any page (not delivery or payment pages) 
  Then I should see the following CTAs in the header (order of left to right): hamburger menu ,search icon ,Debenhams logo ,SFL heart icon ,Mini bag icon

@manual @DDAI-917
Scenario: Validate header on beauty psp page 
  Given I am on any page (not delivery or payment pages) 
  Then I should see the following CTAs in the header (order of left to right): hamburger menu ,search icon ,Debenhams logo ,SFL heart icon ,Mini bag icon

@manual @DDAI-917
Scenario: Validate header on beauty brand room page page 
  Given I am on any page (not delivery or payment pages) 
  Then I should see the following CTAs in the header (order of left to right): hamburger menu ,search icon ,Debenhams logo ,SFL heart icon ,Mini bag icon 

@manual @DDAI-917 @NeedToUpdateExamplesWithPSPList 
Scenario Outline: Validate header on PSP page 
...
@manual @DDAI-917 @NeedToUpdateExamplesWithTCATList 
Scenario Outline: Validate header on TCAT page 
...
@manual @DDAI-917 
Scenario: Validate header on TCAT page 
  Given I am on order confirmation page  
  Then I should see the following CTAs in the header (order of left to right): hamburger menu ,search icon ,Debenhams logo ,SFL heart icon ,Mini bag icon


@manual @DDAI-917
Scenario: Validate SFL in the header with no items 
  Given I have no items SFL 
  Then I should see SFL heart as transparent with a grey border

@manual @DDAI-917
Scenario: Validate SFL in the header with 1 item already added as registered user
  Given I logged in
...
@manual @DDAI-917
Scenario: Validate SFL in the header with 0 item and add product from PDP
...
@manual @DDAI-917
Scenario: Validate SFL in the header with 0 item and add product from My Bag
  Given I have 0 product in SFL 
...
@manual @DDAI-917
Scenario: Validate SFL count increase in the header with 1 item already added
...
@manual @DDAI-917
Scenario: Validate SFL count  decrease in the header with two item already added
...
@manual @DDAI-917
Scenario: Validate SFL page after clicking on heart icon from header if SFL is empty
...
@manual @DDAI-917
Scenario: Validate SFL page after clicking on heart icon from header if user have some products already added
  Given I am on any page 
...
@manual @DDAI-917
Scenario: Validate SFL products merge after login , if guest user saved something in SFL and then login
...
@manual @DDAI-917
Scenario: Validate SFL if registered user have large volume of products in SFL   
