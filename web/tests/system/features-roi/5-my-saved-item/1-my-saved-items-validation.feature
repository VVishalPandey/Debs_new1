Feature: Test Debenhams my saved items for ROI
...
@manual @DDAI-289
Scenario: Navigate to Debenhams my saved items>Validate furniture product on DOTIE where we  can selec color and feet option
  Given I am on my saved items page
  When I have saved a furniture item
  Then I see a choose item options CTA
  And once I select it I can choose a colour (fabric) option (as per PDP)
  Then a feet option (as per PDP)
  Then add to bag
...
@manual @DDAI-494 
Scenario: Navigate to Debenhams my saved items>Validate size ordering for saved items
  Given I Am on the UK, ROI or INT Debenhams mobile site 
  When I saved an item of lingerie (Bras) or clothing (jeans/trousers/tops) 
  And I want to select an option to add to bag from My Saved Items
  Then I should see the sizes listed in ascending order (smallest to largest)
  And I must be able to select only options that are in stock 
  And I must be able to add my option selection to bag
