...
Scenario Outline: Navigate to Debenhams email reminder page from forgot password and validate page when selected continue shopping
  ...
  # Then I should see "titleOfPage" as "Debenhams UK - Fashion, Beauty, Gifts, Furniture & Electricals" on "homeURL"
  Then I should not see "signInButon" on "emailReminderPage"
  And I should not see "goShoppingButon" on "emailReminderPage"
...
@manual @DDAI-324
Scenario: Validate that the email is recived after forgot password and on clicking the link able to set password
  Given I navigate to "loginUrl"
  And I navigate to forgot password page and provided valid email
