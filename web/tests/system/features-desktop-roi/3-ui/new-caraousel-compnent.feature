...
Scenario Outline: validate  New responsive carousel component  on Tcat Pages
  ...
  And I should see arrows on  top  of the teaser
  Then on clicking the arrows, carausel should scroll accoridingly.
  When we move to next spot after clicking on the arrow
  Then we should see teaser on both side of the spots
  And I should see arrows on both teaser
  When I scroll to the last spots
  Then I should see the teaser at the left side of the spot
  And I should see the left arrow on top of the teaser
  And I should see maximum 3 and minimum 0 line inside each spots below heading
...
@manual @DEBD-4110
Scenario: validate the new responsive component with all three type of text postioning(Left/right/center)  on TCATs/content pages/EPSP pages
...
@manual @DEBD-4100
Scenario: Validate the filter accordion icon  changes on content pages does not regress the filtering and  filter icons on other pages
