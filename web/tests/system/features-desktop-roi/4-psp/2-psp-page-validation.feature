...
@automated @DEBD-730 @DEBD-680
Scenario: Navigate to Debenhams women all dresses large image psp page and validate the page
...
Scenario: Navigate to Debenhams normal size image with less than 210 and more than 60 products psp page and validate the page
  When I navigate to "pspPageMoreThan60Products"
  When I "validate & click" subFilter option "Beanie hats" under "Style" on commonPage bigscreen
  Then I should see "breadcrumbSection" exist on "pspPage"
  And I should see "paginationSectionHeader" exist on "pspPage"
  And I should see "paginationSectionFooter" exist on "pspPage"
...
Scenario: Navigate to Debenhams women dresses psp page and validate sort by Newest
  ...
  And I click on "headerText" on "pspPage"
  Then I should see "url" contains "newness"
  And I should see "filterSection" exist on "pspPage"
  And I should see "paginationSectionHeader" exist on "pspPage"
  And I should see "paginationSectionFooter" exist on "pspPage"
  And I should see "sortDropdown" exist on "pspPage"
...
Scenario Outline: Validate the merch set on the women dress page - shoudl be in different container
  Given I navigate to "pspPageWithSEOAndMerchSet"
...
@manual @DEBD-730 @DEBD-959
Scenario:Validate that the Signin Pop up should appear on click of SFL on pspPage for guest user for first time and not on second psp page
...
Scenario: Validate that an out of stock message, product price should not be displayed for no longer available products on PSP
  ...
  And I should not  see the badge
  And I should see an i icon and 'Out of stock' message both in red

@automated @DEBD-730
Scenario: Navigate to Debenhams women suit & tailoring psp page and validate previous, next and page number text
  When I navigate to "psp2PaginationPageUrl"
  And I click on "nextButton" on "pspPage"
  Then I should see "previousButtonState" "enabled" on "pspPage"
...
Scenario Outline: Validate that we are able to see the header for different categories of PSP pages
  Given I navigate to "homeUKURL"
  Then I "validate & click" country as "IE" from "countryOptions" on "commonPage"
  When I "validate & click" subMenu option "<subMenu>" under "<menuOption>" on commonPage bigscreen
  Then I should see "headerText" containing "<headerText>" on "pspPage"
  And I should see "url" contains "<urlContains>"

    Examples:
    | menuOption   | subMenu                 | headerText                   | urlContains                   |
    | Women        | Shoes                   | Shoes                        | /women/shoes-boots/shoes      |
    | Lingerie     | Shorts                  | Knickers                     | /lingerie/knickers/shorts     |
    | Men          | Bags                    | Bags                         | /men/accessories/bags                    |
    | Kids         | All girls' clothes      | Kids' clothing & accessories | /kids/girls                   |
    | Home         | Baking                  | Baking                       | /home/kitchenware/baking     |
    | Gifts & Toys | Soft toys               | Soft toys                    | toys/soft-toys  |
    | Offers       | Luggage sale              | Luggage      | sale/home/luggage  |
    | Electricals  | Heating                 | Heating                      | /electricals/heating          |

@manual @DEBD-730
Scenario: Validate that we are not able to see the header for the brand room PSP pages i.e : Kat von d,Benefit etc.

@notAutomated @DEBD-917
