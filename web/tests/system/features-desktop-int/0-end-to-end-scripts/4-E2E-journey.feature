...
Scenario: Validate that user is still on saved item page and coffee jar still under saved items list
...
Scenario Outline: Validate the shop more tags and select KITCHEN & FOOD STORAGE tag to navigate to psp page
  ...
  And I should see "url" contains "/en-au/home/kitchenware/kitchen-food-storage"
...
Scenario: Validate the filter applied for price less than 15.0
  ...
  And I should see "headerText" containing "KITCHEN & FOOD STORAGE" on "pspPage"
...
@automatedSanity @preprodSanity @e2e
Scenario: Navigate to Debenhams payment page and validate the billing address section   When I click on "proceedPaymentButton" on "deliveryOptionPage"
