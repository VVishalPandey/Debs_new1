...
@automated @ops-UK @HomePage @Sno-1
...
@automated @ops-UK @HomePage @Sno.-2 @Sno.-3
...
@automated @ops-UK @HomePage @Sno.-4
...
@automated @ops-UK @HomePage @Sno.-5
...
Scenario: Validate that all the HomePage content along with finace banner is displayed
  ...
  Then I should scroll to "financeBannerContainer" on "commonPage"
  #When I am in view of "financeBannerContainer" on "commonPage"
  And I should see "financeBannerContainer" exist on "commonPage"
  And I should see "termsAndConditionLink" exist on "commonPage"
  Then I should see "applyNowLink" exist on "commonPage"
  Then I verify "href" of "applyNowLink" as "financeBannerApplyNowLinkHPage" on "commonPage"
...
Scenario: Verify that the footer menu section option-More From Debenhams and the href links of the options available under it
  Then I "validate & click" option is available in "footerMenuList" as "More from Debenhams" on "commonPage"
  And I "validate" option from "footerMenuList" of "More from Debenhams"  as "footerSubMenuList" as "Debenhams cards" on "commonPage"
  ...
  And I "validate" option from "footerMenuList" of "More from Debenhams"  as "footerSubMenuList" as "The Debenhams Blog" on "commonPage"
...
@automated @ops-UK @HomePage @Sno.-6
...
@automated @ops-UK @HomePage @Sno.-7
...
Scenario Outline: Validate the first store locator details in result area
  ...
  And I should see "headerText" as "Store Finder" on "storeFinderPage"
  And I should see "mapImage" exist on "storeFinderPage"
  And I "validate & click" option is available in "storeMenuList" as "Bra Fit Service" on "storeFinderPage"
  And I "validate & click" option is available in "storeMenuList" as "Personal Shopper" on "storeFinderPage"
