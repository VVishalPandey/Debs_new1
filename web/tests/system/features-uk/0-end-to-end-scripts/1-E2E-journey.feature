...
@automatedSanity1 @preprodSanity @e2e
Scenario: Validate the search result when user choose Denby under Brand
  ...
  Then I should see "headerText" containing "Home" on "pspPage"
...
Scenario: Validate the search result when user selected Make up under Categories on brand room of Kat Von D
  ...
  And I "validate & click" option from "brandRoomFilterMenuList" of "Categories"  as "brandRoomFilterSubMenuLinkList" as "Make up" on "pspPage"
  Then I should see "headerText" containing "Makeup" on "pspPage"
  And I should see "url" contains "beauty/make-up/kvd-vegan-beauty#filter"

@automatedSanity1 @preprodSanity @e2e
Scenario: Validate that user lands on TCAT Page if search for Men
 When I click on "searchIcon" on "commonPage"
 And I enter "Gifts" in "searchField" on "commonPage"
 And I click on "searchApply" on "commonPage"
 And I should see "headerText" containing "Gifts" on "pspPage"
 And I should see "url" contains "/gifts"
...
@automatedSanity1 @preprodSanity @e2e
Scenario: Validate that user lands on SCAT if search for Home Sale with variation 1
...
@automatedSanity1 @preprodSanity @e2e
Scenario: Validate that user lands on SCAT if search for Home Sale with variation 3
