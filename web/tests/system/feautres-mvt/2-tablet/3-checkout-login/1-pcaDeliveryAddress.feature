Feature: Verify the functionality of PCA on Delivery Page on Mobile (UK ONLY)

@manual @DBNHST-2336
Scenario: Navigate to Delivery page as Guest or Ghost Home delivery journey when no delivery address is saved and validate the address section default
...
@manual @DBNHST-2336
Scenario: Navigate to Delivery page as Guest or Ghost Home delivery journey when no delivery address is saved and validate the address section when enter address manually selected
  Given I am on Delivery page as Guest or Ghost Home delivery journey
  Then I should see Header text as "Where are we delivering to?"
...
Scenario: Validate PCA address field should not be there and it should have the default delivery address form with postcode lookup
  Given PCA should not work for tablet INT
  Then validate all the validation as in above scenarios
  Given PCA should not work for tablet ROI
  Then validate all the validation as in above scenarios
