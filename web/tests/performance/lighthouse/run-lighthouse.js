 ...
 *
 * * Runs Lighthouse against a config file of URLS located in `tests/performance/
 * lighthouse/lighthouse-urls.json` inside headless chrome
 * Use by `npm run test:lighthouse <environment> <preview> <headless>
 *
 */
...
const isHeadless = process.argv[4] || 'false'
console.log(`npm run test:lighthouse "${configFile}" "${environment}" "${isPreview}" "${isHeadless}"`)
...
const runLighthouse = function(env, isPreview, isHeadless) {
    let arg = 'prod'
    ...
    for (const page in env) {
        if (env.hasOwnProperty(page)) {
            console.log(`npm run test:pwa-${arg} ${env[page].url} ${page} ${isHeadless}`)
            commands.push(`npm run test:pwa-${arg} ${env[page].url} ${page} ${isHeadless}`)
        }
    }
...
const runParseUrls = function(configJson, environment, isPreview, isHeadless) {
    try {
        let config = []
        ...
        runLighthouse(env, isPreview, isHeadless)
    } catch (e) {
        console.log(e)
...
runParseUrls(configFile, environment, isPreview, isHeadless)
